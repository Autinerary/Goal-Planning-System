 ...
 *   POST { value: -1 | 0 | 1 } — cast / change / retract a vote on an answer.
 */
...
export async function POST(
  ...
  if (error) {
    console.error('[community] vote answer failed:', error.message);
  ...
  const { data: answer } = await client
    ...
    .eq('id', params.id)
    .maybeSingle();
  return NextResponse.json({ ok: true, ...(answer ?? {}), viewer_vote: value });
}
