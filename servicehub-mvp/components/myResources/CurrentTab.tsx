...
export default function CurrentTab({ userId }: CurrentTabProps) {
  ...
  const fetchCurrentResources = async () => {
    setLoading(true)
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch current resources')
      }
  ...
  const handleMoveToPast = async (resourceId: string) => {
    try {
