  ...
  useEffect(() => {
    ...
    if (agentMilestone) {
      ...
      setMilestone({
        ...
        status: agentMilestone.status || 'in_progress',
        progress: agentMilestone.progress ?? 0,
        race: agentMilestone.race,
        nextSteps: agentTasks.length ? agentTasks : (agentMilestone.nextSteps || []),
        tools: agentTools,
      })
    } else if (mockMilestones[milestoneId]) {
      setMilestone(mockMilestones[milestoneId])
    } else {
      setMilestone({
        id: milestoneId,
    ...
    setLoading(false)
  }, [milestoneId, pathPlanning, toolRecommendation])
